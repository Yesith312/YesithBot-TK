...
global.authFile = `JoanBotSession`
...
mobile: MethodMobile, 
browser: opcion == '1' ? ['JoanBot-TK', 'Edge', '2.0.0'] : methodCodeQR ? ['JoanBot-TK', 'Edge', '2.0.0'] : ['Ubuntu', 'Edge', '110.0.1587.56'],
auth: {
creds: state.creds,
...
if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', "JoanJadiBot"], tmp.forEach(filename => cp.spawn('find', [filename, '-amin', '2', '-type', 'f', '-delete'])))}, 30 * 1000)}
...
if (reason == 405) {
await fs.unlinkSync("./JoanBotSession/" + "creds.json")
...
let prekey = []
let directorio = readdirSync("./JoanBotSession")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-')
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./JoanBotSession/${files}`)
})
} 

function purgeSessionSB() {
try {
const listaDirectorios = readdirSync('./JoanJadiBot/');
let SBprekey = [];
listaDirectorios.forEach(directorio => {
if (statSync(`./JoanJadiBot/${directorio}`).isDirectory()) {
const DSBPreKeys = readdirSync(`./JoanJadiBot/${directorio}`).filter(fileInDir => {
return fileInDir.startsWith('pre-key-')
})
SBprekey = [...SBprekey, ...DSBPreKeys];
DSBPreKeys.forEach(fileInDir => {
if (fileInDir !== 'creds.json') {
unlinkSync(`./JoanJadiBot/${directorio}/${fileInDir}`)
}})
}})
...
const directories = ['./JoanBotSession/', './JoanJadiBot/']
