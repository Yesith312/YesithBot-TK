...
global.authFile = `YesithBotSession`
...
mobile: MethodMobile, 
browser: opcion == '1' ? ['YesithBot-TK', 'Edge', '2.0.0'] : methodCodeQR ? ['YesithBot-TK', 'Edge', '2.0.0'] : ['Ubuntu', 'Edge', '110.0.1587.56'],
auth: {
creds: state.creds,
...
if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', "YesithJadiBot"], tmp.forEach(filename => cp.spawn('find', [filename, '-amin', '2', '-type', 'f', '-delete'])))}, 30 * 1000)}
...
if (reason == 405) {
await fs.unlinkSync("./YesithBotSession/" + "creds.json")
...
let prekey = []
let directorio = readdirSync("./YesithBotSession")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-')
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./YesithBotSession/${files}`)
})
} 

function purgeSessionSB() {
try {
const listaDirectorios = readdirSync('./YesithJadiBot/');
let SBprekey = [];
listaDirectorios.forEach(directorio => {
if (statSync(`./YesithJadiBot/${directorio}`).isDirectory()) {
const DSBPreKeys = readdirSync(`./YesithJadiBot/${directorio}`).filter(fileInDir => {
return fileInDir.startsWith('pre-key-')
})
SBprekey = [...SBprekey, ...DSBPreKeys];
DSBPreKeys.forEach(fileInDir => {
if (fileInDir !== 'creds.json') {
unlinkSync(`./YesithJadiBot/${directorio}/${fileInDir}`)
}})
}})
...
const directories = ['./YesithBotSession/', './YesithJadiBot/']
